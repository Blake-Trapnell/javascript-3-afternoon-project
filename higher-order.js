...
//Code Here
let totalPopulation  = populations.reduce((acc, el)=>{
  return acc += el
})
...
//Code Here
let myStrongest = monstersInYourPocket.filter((el,i,ar)=> {
  if (el.CP > 200){
    return true
  }
})
...
let orderTotals = orders.map(function(el, i, arr) {
  let taxPerc = 1 + el.tax
  return el.price * taxPerc
})
...
let bobsTotal = purchases.reduce((acc,el,i,)=> {
